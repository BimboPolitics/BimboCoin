...
const char * const BITCOIN_CONF_FILENAME = "bimbocoin.conf";
const char * const BITCOIN_PID_FILENAME = "bimbocoind.pid";
...
#else
    const char* pszModule = "bimbocoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BimboCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BimboCoin";
#else
    // Unix
    return pathRet / ".bimbocoin";
#endif
#endif
}
