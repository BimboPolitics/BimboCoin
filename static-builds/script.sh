...
if [ ! -f ~/db4/lib/libdb-4.8.a ]
then
    echo "Please run install_db4.sh first from ~/"
	exit 1
fi
...
apt install -y curl g++-aarch64-linux-gnu g++-7-aarch64-linux-gnu gcc-7-aarch64-linux-gnu binutils-aarch64-linux-gnu g++-arm-linux-gnueabihf g++-7-arm-linux-gnueabihf gcc-7-arm-linux-gnueabihf binutils-arm-linux-gnueabihf g++-7-multilib gcc-7-multilib build-essential libtool autotools-dev automake pkg-config libssl-dev libevent-dev bsdmainutils python3 curl libboost-system-dev libboost-filesystem-dev libboost-chrono-dev libboost-test-dev libboost-thread-dev libboost-all-dev libboost-program-options-dev libminiupnpc-dev libzmq3-dev libgmp3-dev libqt5gui5 libqt5core5a libqt5dbus5 qttools5-dev qttools5-dev-tools libprotobuf-dev protobuf-compiler libqrencode-dev unzip doxygen cmake nsis wine-stable wine64 bc binutils-gold git pkg-config autoconf ca-certificates python g++ mingw-w64 g++-mingw-w64 nsis zip rename librsvg2-bin libtiff-tools imagemagick libcap-dev libz-dev libbz2-dev python-dev python-setuptools fonts-tuffy
cd ~/

# Removes any existing builds and starts clean WARNING
rm -rf ~/BimboCoin ~/sign ~/release

git clone https://github.com/BimboPolitics/BimboCoin
cd ~/BimboCoin
git checkout $BRANCH
...
find ${DISTNAME}/bin -type f -executable -exec ../BimboCoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
find ${DISTNAME}/lib -type f -exec ../BimboCoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make dist
SOURCEDIST=`echo bimbo-*.tar.gz`
mkdir -p ~/BimboCoin/temp
cd ~/BimboCoin/temp
tar xf ../$SOURCEDIST
find bimbo-* | sort | tar --no-recursion --mode='u+rw,go+r-w,a+X' --owner=0 --group=0 -c -T - | gzip -9n > ../$SOURCEDIST
cd ~/BimboCoin
...
cp -f bimbo-*setup*.exe ~/release/unsigned/
mkdir -p ~/win64
...
cd ~/
rm -rf win64
cp -rf BimboCoin/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir -p unsigned
mv ~/BimboCoin/bimbo-*setup-unsigned.exe unsigned/
...
cd ~/
rm -rf win32
cp -rf BimboCoin/contrib/windeploy ~/sign/win32
...
mkdir -p ~/BimboCoin/depends/SDKs
cp ~/MacOSX10.11.sdk.tar.gz ~/BimboCoin/depends/SDKs/MacOSX10.11.sdk.tar.gz
cd ~/BimboCoin/depends/SDKs && tar -xf MacOSX10.11.sdk.tar.gz 
...
cd ~/BimboCoin
make deploy
$PWD/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Bimbo-Core.dmg" ~/release/unsigned/$DISTNAME-osx-unsigned.dmg
