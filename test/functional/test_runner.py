...
def main():
    ...
    tmpdir = "%s/bimbocoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and bimbocoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "bimbocoind"]) is not None:
            print("%sWARNING!%s There is already a bimbocoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "BIMBOCOIND" not in os.environ:
        os.environ["BIMBOCOIND"] = build_dir + '/src/bimbocoind' + exeext
        os.environ["BIMBOCOINCLI"] = build_dir + '/src/bimbocoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `bimbocoin-cli help` (`rpc_interface.txt`).
